...
function drawTacosPerformance(id,d){
  ...
  const labels=points.map((x,i)=>`<text x="${X(i)}" y="${h-11}" text-anchor="middle" class="light-axis">${x.label}</text>`).join('');
  const dots=c.map((a,i)=>`<circle cx="${a[0]}" cy="${a[1]}" r="4.3" fill="#fff" stroke="#4E79FF" stroke-width="2.4"/>
...
function drawPremiumTraffic(id,weeks){
  ...
  const dots=c.map((a,i)=>`<circle cx="${a[0]}" cy="${a[1]}" r="4.3" fill="#fff" stroke="#F47322" stroke-width="2.5"/><text x="${a[0]}" y="${Math.max(16,a[1]-12)}" text-anchor="middle" class="dark-value">${fmtNum(weeks[i].sessions)}</text>`).join('');
  el.innerHTML=`<svg viewBox="0 0 ${w} ${h}">
...
function drawPremiumConversion(id,weeks,target){
  ...
  const labels=weeks.map((x,i)=>`<text x="${X(i)}" y="${h-10}" text-anchor="middle" class="dark-axis">${escapeHtml(shortLabel(x.label))}</text>`).join('');
  const dots=c.map((a,i)=>`<circle cx="${a[0]}" cy="${a[1]}" r="4.3" fill="#fff" stroke="#4B7CF3" stroke-width="2.5"/><text x="${a[0]}" y="${Math.max(16,a[1]-12)}" text-anchor="middle" class="dark-value">${fmtPct(weeks[i].conversion)}</text>`).join('');
